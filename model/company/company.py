...
class Company(SQLModel, table=True):
    ...
    gics_sector_id: Optional[str] = Field(foreign_key="GICSSector.id")
    gics_sector: Optional["GICSSector"] = Relationship(back_populates="companies")

    gics_industry_group_id: Optional[str] = Field(foreign_key="GICSIndustryGroup.id")
    gics_industry_group: Optional["GICSIndustryGroup"] = Relationship(back_populates="companies")

    gics_industry_id: Optional[str] = Field(foreign_key="GICSIndustry.id")
    gics_industry: Optional["GICSIndustry"] = Relationship(back_populates="companies")

    gics_subindustry_id: Optional[str] = Field(foreign_key="GICSSubIndustry.id")
